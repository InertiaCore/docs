...
const frameworkIcons = {
  ...
  laravel: (
    <svg viewBox="0 0 16 16" width="16" height="16" style={{ marginRight: 6, verticalAlign: 'middle' }}>
      <rect width="16" height="16" rx="3" fill="#FF2D20" />
      <path d="M5.6 3.8h1.5v7.2h3.3v1.2H5.6V3.8z" fill="#fff" />
    </svg>
  ),
  dotnet: (
    <svg viewBox="0 0 16 16" width="16" height="16" style={{ marginRight: 6, verticalAlign: 'middle' }}>
      <rect width="16" height="16" rx="3" fill="#512BD4" />
...
export function CodeGroup({ children }) {
  ...
  // Default to the .NET tab when the user hasn't picked a framework yet.
  const dotnetTab = tabs.find((t) => t?.icon === 'dotnet');
  const defaultValue = dotnetTab ? dotnetTab.title || 'tab-dotnet' : undefined;

  return (
    <Tabs groupId="code-group" defaultValue={defaultValue}>
