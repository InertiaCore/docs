    ...
    public IActionResult Index()
    {
        return Inertia.Render("Users/Index", new
        {
            Users = context.Users.Select(u => new
            {
                ...
                u.Name,
                u.Email,
                EditUrl = Url.Action(nameof(Edit), new { id = u.Id }),
            }).ToList(),
            CreateUrl = Url.Action(nameof(Create)),
        });
    }

    [HttpGet("/users/create")]
    public IActionResult Create() => Inertia.Render("Users/Create");

    [HttpGet("/users/{id:int}/edit")]
    public IActionResult Edit(int id) => Inertia.Render("Users/Edit", new { Id = id });
}
```
...
```csharp .NET icon="dotnet"
// In your HandleInertiaRequests middleware (or Program.cs), register a custom resolver.
Inertia.ResolveUrlUsing(actionContext =>
{
    // Return the URL for the request...
    return actionContext.HttpContext.Request.Path;
});
```
