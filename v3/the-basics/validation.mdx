...
InertiaCore (the ASP.NET Core adapter) automatically forwards every `ModelState` error as the `errors` prop, so [Data Annotations](https://learn.microsoft.com/aspnet/core/mvc/models/validation) and [FluentValidation](https://docs.fluentvalidation.net/) both work without any manual wiring. Just decorate your request model and re-render the action on `!ModelState.IsValid`:
...
public IActionResult Store([FromBody] CreateUserRequest request)
{
    ...
    {
        // Errors are flashed to TempData and forwarded to the response automatically.
        return Index();
    }
    // ...
