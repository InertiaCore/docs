...
// Wayfinder is a Laravel-specific integration. InertiaCore has no
// equivalent, so pass the target component name explicitly on the
// client when performing an instant visit.
```
...
```csharp .NET icon="dotnet"
// InertiaCore always emits the sharedProps metadata key in the page
// response, so shared props are carried over during instant visits
