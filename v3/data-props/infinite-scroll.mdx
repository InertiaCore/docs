...
public IActionResult Index([FromQuery] int page = 1)
{
    ...
    var metadata = new ScrollMetadata(
        pageName: "page",
    ...
    {
        Users = Inertia.Scroll(data, metadata: metadata)
    });
}
```
...
public IActionResult Dashboard(
    [FromQuery(Name = "users")] int usersPage = 1,
    [FromQuery(Name = "orders")] int ordersPage = 1)
{
    const int perPage = 15;
    ...
    {
        Users = Inertia.Scroll(users, metadata: new ScrollMetadata("users", currentPage: usersPage)),
        Orders = Inertia.Scroll(orders, metadata: new ScrollMetadata("orders", currentPage: ordersPage))
    });
}
```
...
```csharp .NET icon="dotnet"
// Provide a paginated payload plus metadata describing page boundaries...
Inertia.Scroll(
    new { data = users },
    metadata: new ScrollMetadata("page", currentPage: page, nextPage: page + 1)
);
```
...
// Provide custom metadata resolution...
Inertia.Scroll(customPaginatedData, metadata: metadataProvider);
```
...
public class FractalScrollMetadata : IScrollMetadata
{
    private readonly object _resource;

    public FractalScrollMetadata(object resource) => _resource = resource;

    public string GetPageName() => "page";
    public object? GetPreviousPage() => null;
    public object? GetNextPage() => null;
    public object? GetCurrentPage() => null;
}
```
...
// Using a callback for deferred evaluation...
Inertia.Scroll(() => TransformData(data), metadata: new FractalScrollMetadata(data));
```
...
```csharp .NET icon="dotnet"
// Create a helper method in a base controller or service...
public static ScrollProp FractalScroll(object data)
{
    return Inertia.Scroll(data, metadata: new FractalScrollMetadata(data));
}

// Then use it in your controllers...
return Inertia.Render("Users/Index", new
{
    Users = FractalScroll(fractalCollection)
});
```
