...
// In Program.cs. AddViteHelper() + UseInertia() auto-wires
// Inertia.Version(Vite.GetManifestHash) from your Vite manifest.
...
// Pin a fixed version to disable automatic asset-driven reloads,
// then expose the real asset version as shared data.
Inertia.Version("1.0.0");

Inertia.Share("version", Vite.GetManifestHash());
```
