...
Each HTTP method accepts its own generic type parameter, allowing you to override the response type on a per-request basis. This is useful when different endpoints return different response shapes.
...
const submitted: UserResponse = await http.submit<UserResponse>("post", "/api/users");
