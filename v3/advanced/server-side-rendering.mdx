...
You may also prevent the Laravel adapter from dispatching SSR requests programmatically using the `Inertia::disableSsr()` method. This is useful for [disabling SSR during tests](/v3/advanced/testing#disabling-ssr-during-tests) while the Vite dev server is active, as SSR requests would otherwise be dispatched to the dev server's endpoint.
