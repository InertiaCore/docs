...
{
    if (new[] { 403, 404, 500, 503 }.Contains(response.StatusCode()))
    {
        response
            .Render("ErrorPage", new { status = response.StatusCode() })
            .WithSharedData();
    }
});

// Register the Inertia exception handler early in the pipeline so it can
// intercept unhandled exceptions on Inertia requests before the response
// body has started writing.
...
public class ErrorController : Controller
{
    ...
    public IActionResult Handle(int statusCode)
    {
        if (new[] { 500, 503, 404, 403 }.Contains(statusCode))
        {
            ...
            return Inertia.Render("ErrorPage", new { Status = statusCode });
        }
