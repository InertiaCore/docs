...
// Enable globally via InertiaOptions in Program.cs:
//   builder.Services.AddInertia(options => options.EncryptHistory = true);
// Then opt a specific action out by passing false before returning a response.
Inertia.EncryptHistory(false);
```
...
// Enable globally on the whole pipeline:
app.UseInertiaEncryptHistory();

// Or scope the built-in EncryptHistoryMiddleware to a branch of routes:
app.UseWhen(
    ctx => ctx.Request.Path.StartsWithSegments("/admin"),
    branch => branch.UseMiddleware<EncryptHistoryMiddleware>());

// Or apply the [EncryptHistory] attribute to a controller or action:
[EncryptHistory]
public class AdminController : Controller
{
    public IActionResult Index() => Inertia.Render("Admin/Index");
}
```
