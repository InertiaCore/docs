...
public class PodcastsControllerTest : IClassFixture<WebApplicationFactory<Program>>
{
    private readonly HttpClient _client;

    public PodcastsControllerTest(WebApplicationFactory<Program> factory)
    {
    ...
    {
        // Make a request with the X-Inertia header to get JSON
        var request = new HttpRequestMessage(HttpMethod.Get, "/podcasts/41");
        request.Headers.Add("X-Inertia", "true");
        request.Headers.Add("X-Inertia-Version", Inertia.GetVersion());
        ...
        Assert.Equal("Podcasts/Show", page.GetProperty("component").GetString());
        Assert.Equal("The Laravel Podcast",
            page.GetProperty("props").GetProperty("podcast").GetProperty("subject").GetString());
    }
}
```
...
```csharp .NET icon="dotnet"
// Assert the JSON response contains the expected properties
var props = page.GetProperty("props");

Assert.True(props.TryGetProperty("podcast", out _));
Assert.True(props.GetProperty("podcast").TryGetProperty("id", out _));
```
...
// Checking if a root-level property has 7 items...
Assert.Equal(7, props.GetProperty("podcasts").GetArrayLength());
...
Assert.True(message.TryGetProperty("subject", out _));
Assert.Equal(5, message.GetProperty("comments").GetArrayLength());

var firstComment = message.GetProperty("comments")[0];
Assert.True(firstComment.TryGetProperty("body", out _));
Assert.Equal(1, firstComment.GetProperty("files").GetArrayLength());
Assert.True(firstComment.GetProperty("files")[0].TryGetProperty("url", out _));
```
...
Assert.True(comments[0].TryGetProperty("body", out _));
```
...
// Assert against deeply nested values...
...
// Assert only specific properties exist, ignoring others
Assert.True(message.TryGetProperty("subject", out _));
Assert.True(message.TryGetProperty("comments", out _));
```
...
Assert.True(message.TryGetProperty("subject", out _));
Assert.False(message.TryGetProperty("published_at", out _));
```
