...
On the InertiaCore side, accept the upload as `IFormFile` on a `[FromForm]` model. ASP.NET Core handles the multipart parsing and ModelState validation, and InertiaCore forwards any errors as the `errors` prop:
...
public async Task<IActionResult> Update(int id, [FromForm] UserUpdateRequest request)
{
    ...
    {
        user.AvatarPath = await _files.SaveAsync(request.Avatar);
    }
