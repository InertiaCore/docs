...
// In Program.cs, configure SSR options:
...
// To disable SSR for specific routes, use middleware:
public class DisableSsrMiddleware
{
    private readonly RequestDelegate _next;

    public DisableSsrMiddleware(RequestDelegate next)
    {
        _next = next;
    }
    ...
    {
        // SSR can be configured globally via
        // InertiaOptions.SsrEnabled in Program.cs
        await _next(context);
    }
}
```
...
```bash .NET icon="dotnet"
# Kill the Node.js SSR process directly
kill $(lsof -t -i:13714)
```
...
```bash .NET icon="dotnet"
# Check if the SSR server is responding
