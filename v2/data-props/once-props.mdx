...
// InertiaCore does not have a direct Inertia.Once() method.
// Use Inertia.Lazy() for similar deferred evaluation behavior.
return Inertia.Render("Billing", new
{
    Plans = Inertia.Lazy(() => _context.Plans.ToList())
});
```
...
```csharp .NET icon="dotnet"
// No direct equivalent in InertiaCore.
// Use Inertia.Lazy() to evaluate data only when explicitly requested.
return Inertia.Render("Billing", new
{
    Plans = Inertia.Lazy(() => _context.Plans.ToList())
});
...
```csharp .NET icon="dotnet"
// No direct equivalent in InertiaCore.
// Conditionally include the prop based on your logic.
return Inertia.Render("Billing", new
{
    Plans = Inertia.Lazy(() => _context.Plans.ToList())
});
...
```csharp .NET icon="dotnet"
// No direct equivalent in InertiaCore.
// Use Inertia.Lazy() and manage caching/expiration separately.
return Inertia.Render("Dashboard", new
{
    Rates = Inertia.Lazy(() => _context.ExchangeRates.ToList())
});
```
...
{
    MemberRoles = Inertia.Lazy(() => _context.Roles.ToList())
});

// Invite form...
return Inertia.Render("Team/Invite", new
{
    AvailableRoles = Inertia.Lazy(() => _context.Roles.ToList())
});
```
...
```csharp .NET icon="dotnet"
Inertia.Share("countries", Inertia.Lazy(() => _context.Countries.ToList()));
```
...
```csharp .NET icon="dotnet"
// No direct ShareOnce equivalent in InertiaCore.
Inertia.Share("countries", Inertia.Lazy(() => _context.Countries.ToList()));
```
...
```csharp .NET icon="dotnet"
// No direct equivalent in InertiaCore. Use Inertia.Share() with Inertia.Lazy().
Inertia.Share("countries", Inertia.Lazy(() => _context.Countries.ToList()));
```
...
```csharp .NET icon="dotnet"
// In your middleware or Program.cs, share data globally using Inertia.Share().
Inertia.Share("countries", Inertia.Lazy(() => _context.Countries.ToList()));
```
...
return Inertia.Render("Dashboard", new
{
    Permissions = Inertia.Defer(() => _context.Permissions.ToList()),
    Activity = Inertia.Merge(() => user.RecentActivity()),
    Categories = Inertia.Optional(() => _context.Categories.ToList())
});
```
