...
{
    var users = _context.Users.Paginate(Request);
    var orders = _context.Orders.Paginate(Request);

    return Inertia.Render("Dashboard", new
    {
        Users = Inertia.Merge(users),
        Orders = Inertia.Merge(orders)
    });
}
```
...
```csharp .NET icon="dotnet"
// Use Inertia.Merge() for mergeable pagination data...
Inertia.Merge(_context.Users.Paginate(Request, perPage: 20));
```
...
```csharp .NET icon="dotnet"
// Use Inertia.Merge() with your custom paginated data...
Inertia.Merge(customPaginatedData);
```
...
```csharp .NET icon="dotnet"
// InertiaCore does not have a direct equivalent of ProvidesScrollMetadata.
// Use Inertia.Merge() with your own pagination metadata.
```
...
```csharp .NET icon="dotnet"
// Use Inertia.Merge() with a callback for deferred evaluation...
Inertia.Merge(() => TransformData(data));
```
...
public static MergeProp MergeCollection(object data)
{
...
return Inertia.Render("Users/Index", new
{
    Users = MergeCollection(fractalCollection)
});
```
