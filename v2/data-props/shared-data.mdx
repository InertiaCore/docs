...
// Register shared data once at startup...
Inertia.Share("appName", builder.Configuration["AppName"]);

// ...or resolve per-request inside middleware, using Inertia.Lazy
// so the value is only evaluated when needed.
app.Use(async (context, next) =>
{
    Inertia.Share("auth.user", Inertia.Lazy(() => context.User.Identity?.IsAuthenticated == true
...
// In .NET, call Inertia.ShareOnce to register a once prop by key.
