...
Add `AddAntiforgery()` and `UseAntiforgery()` to validate the CSRF token on non-`GET` requests. To turn token-mismatch failures into a friendly Inertia redirect-back instead of a raw 400, see the [CSRF protection](/v2/security/csrf-protection) page.

## Exception handling

Wire an exception handler to render an InertiaCore error page on uncaught exceptions. The recommended pattern uses `Inertia.HandleExceptionsUsing(...)` + `app.UseInertiaExceptionHandler()` — see the [error handling](/v2/advanced/error-handling) page for the full snippet.
